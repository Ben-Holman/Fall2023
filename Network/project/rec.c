...
int n, len, m; //used for the length of the message
...
m = bind(soc, (const struct sockaddr *) &self, sizeof(self)); /* (C)
...
m = recvfrom(soc, buf, sizeof(buf), 0, (struct sockaddr *)&peer, &len);
...
buf[m] = '\0'; /* attach the \0 at the end of
the stream of characters since buf is a C-string */
printf("Datagram received = %s\n", buf);
