...
// receiver.c
// receiver process example - datagram socket
...
#include <stdio.h>
#include <stdlib.h> // not used here but possibly required in your project
#include <sys/types.h>
...
struct sockaddr_un peer; // declare another socket address
int n, len; //used for the length of the message
...
strcpy(self.sun_path, NAME); //copy the NAME to the socket pathname of self
...
*/
n = bind(soc, (const struct sockaddr *) &self, sizeof(self)); /* (C)
*/
...
buf[n] = '\0'; /* attach the \0 at the end of
the stream of characters since buf is a C-string */
printf("Datagram received = %s\n", buf); /* print buf which now
contains the datagram*/
cleanup(soc, self.sun_path); // close the socket and disconnect

//SENDER CODE 
int soc_send; // socket number
char buf_send[] = "Hello there, this is the message to be sent to the sender terminal"; // message content to be sent
const char *NAME_SEND = "./receiver_soc"; //name of the socket path
struct sockaddr_un peer_rec; //socket address variable
int n_send;
peer_rec.sun_family = AF_UNIX;// The AF_UNIX (also called the AF_LOCAL) is a
//socket family used to communicate between processes on the same machine.
strcpy(peer_rec.sun_path, NAME_SEND); //copy the NAME to the socket pathname of peer

soc_send = socket(AF_UNIX, SOCK_DGRAM, 0); /* SOCK_DGRAM is used for
datagram oriented sockets */

if ( access(peer_rec.sun_path, F_OK) > -1 ){

n_send = sendto(soc_send, buf_send, strlen(buf_send), 0, (struct sockaddr *)&peer_rec,
sizeof(peer_rec)); /* the datagram is sent and the content of the buf is
transmitted */
...
if ( n_send < 0 ){ 
  ...
  printf("Sender: %d characters sent!\n", n_send); /* print the message
sent */
  close(soc_send); /* close the socket */
  }
